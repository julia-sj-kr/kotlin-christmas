...
- [x] 주문 내용(메뉴이름-갯수)을 입력하는 기능 구현
  - [x] 유효성 검사: 고객이 메뉴판에 없는 메뉴를 입력하는 경우, 메뉴의 개수는 1 이상일때만 유효, 메뉴 형식이 다를 경우, 중복 메뉴일 경우
- [x] 이벤트 메커니즘 구현
  - [x] 크리스마스 디데이 할인
  - [x] 평일 할인
  - [x] 주말 할인
...
- [x] 상세 결제 내역 출력 기능 구현
  - [x] 증정 메뉴 출력 기능 구현
  - [x] 총 혜택 금액(증정 메뉴 금액 포함) 출력 기능 구현
  - [x] 할인 후 예상 결제 금액(할인 전 총주문 금액 - 총 혜택 금액 + 증정 메뉴 금액) 출력 기능 구현
- [x] <12월 이벤트 배지> 출력 기능 구현
- [ ] 주문 가능 조건 및 이벤트 최소 충족 조건 반영
  - 총주문 금액 10,000원 이상부터 이벤트가 적용됩니다.
  - 음료만 주문 시, 주문할 수 없습니다.
  - 메뉴는 한 번에 최대 20개까지만 주문할 수 있습니다.